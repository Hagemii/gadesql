...
class Trainer:
    ...
    def train(self, config, modeldir):
        ...
        with self.data_random:
            ...
            # test_array = ["SELECT", "FROM", "WHERE", "GROUP_BY", "HAVING", "ORDER_BY", "LIMIT" ]
            # for i in range(len(train_data)):
            #     train_data[i][0]['position_code'] = test_array
        ...
        with self.data_random:
            for batch in train_data_loader:
                ...
                if last_step % self.train_config.eval_every_n == 0:
                    self.model.encoder.bert_model.eval()
                    ...
                    if self.train_config.eval_on_val:
                        eval_loss = self._eval_model(self.logger, self.model, last_step, val_data_loader, 'val',
                                         num_eval_items=self.train_config.num_eval_items)
    #            wandb.log({"train_loss": float(train_loss), "eval_loss": float(eval_loss), "step_eval":last_step})
                self.model.encoder.bert_model.train()
     ...
     #           wandb.log({"loss": loss.item(),"step":last_step})
...
def main(args):
    ...
    config["optimizer"]["bert_lr"] = 0.000001
