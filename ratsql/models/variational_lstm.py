...
class RecurrentDropoutLSTMCell(torch.jit.ScriptModule):
    ...
    def _hook_remove_dropout_masks_from_state_dict(cls, instance, state_dict, prefix, local_metadata):
        input_dropout_mask = prefix + '_input_dropout_mask'
        h_dropout_mask = prefix + '_h_dropout_mask'
        test = 'encoder.bert_model.embeddings.position_ids'

        if input_dropout_mask in state_dict:
            del state_dict[input_dropout_mask]
        if h_dropout_mask in state_dict:
            del state_dict[h_dropout_mask]
